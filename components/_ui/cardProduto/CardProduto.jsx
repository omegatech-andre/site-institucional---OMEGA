...
export default function CardProduto({
  ...
  const imagemAnterior = () => {
    setInicio(
      (inicio - 1 + (images.length - numImagens + 1)) %
        (images.length - numImagens + 1)
    );
  };
        ...
        <div className="cardproduto__img">
          ...
          {images.length > 1 && (
            <div className="cardproduto__img--mini">
              <div className="setas">
                {images.length > numImagens && (
                  <div className="bgsetas" onClick={imagemAnterior}>
                    <RiArrowLeftSLine size={30} />
                  </div>
                )}
                {images.length > numImagens && (
                  <div className="bgsetas" onClick={proximaImagem}>
              ...
              <div className="images">
                {imagensVisiveis.map((image, index) => (
                  <img
                    ...
                    src={image}
                    alt=""
                    onClick={() => setImagemPrincipal(image)}
                  />
                ))}
              </div>
            </div>
              ...
              <div className="cardproduto__detalhes--cores">
                {cores.map((objetoCor, index) => (
                  <div key={index}>
                    <div
                      className="coresbox"
                      style={{ backgroundColor: objetoCor.color }}
                    ></div>
