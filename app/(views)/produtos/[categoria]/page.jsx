...
export default function PageProdutos({ params }) {
    ...
    return (
      <>
        <div className="pageprodutos__carregando">
          ...
          <div>
            <a href={`/produtos/${linhaEscolhida}`}>
              <IoMdRefresh size={30} />
            </a>
  ...
  const categoriaAtual = findCategory(
    linhaAtual?.category,
    categoriaSelecionada
  );
  const produtoAtual = findProduct(categoriaAtual?.product, produtoParaBuscar);
  const outrasLinhas = filterOtherLines(
    data.catalog[FIRST_ELEMENT].line,
    linhaEscolhida
  );
      ...
      <div className="pageprodutos">
        ...
        <div className="pageprodutos__content">
          ...
          <div>
            ...
            >
              <option disabled style={{ color: "#d9d9d9" }} value="">
            ...
            <button onClick={() => setProdutoParaBuscar(produtoSelecionado)}>
              buscar produto
            </button>
          </div>
        </div>
